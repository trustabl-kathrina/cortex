...
export interface Config {
  /** Harness home containing the fixed user-global `AGENTS.md`; defaults to `$CORTEX_HOME` or `~/.cortex`. */
  cortexHome?: string
 ...
 * on `ctx.sandboxPolicy` (`@cortex/sandbox-policy`), which resolves
...
export interface Config {
  /** Override the fixture path; defaults to `$CORTEX_SNAPSHOT_FILE`. */
  file?: string
  /** Override the sidecar path; defaults to `$CORTEX_SNAPSHOT_OVERRIDE`. */
  overrideFile?: string
  /**
   * Override the child-log paths; defaults to `$CORTEX_SNAPSHOT_CHILD_FILES` (a
...
export interface Config {
  ...
  /** Cortex config root. Defaults to `$CORTEX_HOME` or `~/.cortex`. */
  cortexHome?: string
  /** Shared agent config root. Defaults to `$CORTEX_AGENTS_HOME` or `~/.agents`. */
  agentsHome?: string
  ...
  /** Bundled skill root; defaults to `$CORTEX_BUNDLED_SKILL_DIR` when default roots are included, otherwise mounts none. */
...
export interface Config {
   ...
   * harness's own `CORTEX_API_KEY`. Forwarded on top of a credential-scrubbed
...
export interface Config {
   ...
   * `@cortex/tool-call-timeout-policy` through `exec.signal`.
   */
  timeoutMs?: number
}
```
