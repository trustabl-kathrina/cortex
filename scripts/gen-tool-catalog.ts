...
import * as ToolSubagentListAgents from '@cortex/tool-subagent-control/list-agents'
import * as ToolSubagentReport from '@cortex/tool-subagent-report'
import SkillRegistry from '@cortex/skill'
import * as SkillFileSystem from '@cortex/skill-filesystem'
import LocalJobRegistry from '@cortex/jobs-local'
import * as ToolAskUser from '@cortex/tool-ask-user'
import * as ToolBash from '@cortex/tool-bash'
import * as ToolPwsh from '@cortex/tool-pwsh'
import * as ToolBashPersistent from '@cortex/tool-bash-persistent'
import CordisHostRunner from '@cortex/cordis-host-runner'
import * as ToolCordis from '@cortex/tool-cordis'
import * as ToolFs from '@cortex/tool-fs'
import * as ToolFsSearch from '@cortex/tool-fs-search'
import * as ToolStrReplaceEditor from '@cortex/tool-str-replace-editor'
...
import * as ToolSessionQuery from '@cortex/tool-session-query'
...
const TOOL_PACKAGES: ToolPackage[] = [
  ...
  },
  {
    ...
    note:
      'The bash tool is the model-facing consumer of the bash executor seam. A `run_in_background` run registers with the generic `ctx.jobs` runtime and is collected/stopped through the `job_*` tools from `@cortex/tool-jobs`; the `enableRunInBackground` config (default true) removes the parameter entirely when disabled.',
  },
  {
    pkg: '@cortex/tool-pwsh',
    dir: 'tool-pwsh',
    ...
    note:
      'The pwsh tool is the PowerShell-dialect consumer of the bash executor seam for Windows compositions (a PowerShell executor such as `@cortex/pwsh-local` backs `ctx.shell`); it mirrors the bash tool call-for-call minus sandbox controls — `run_in_background` runs register with the generic `ctx.jobs` runtime and are collected/stopped through the `job_*` tools, and the managed `CORTEX_*` environment comes from `@cortex/shell-env`. Each call runs in a fresh process (no persistent PTY session), with native `C:\\...` paths and `$env:NAME` variables.',
  },
  {
    pkg: '@cortex/tool-cordis',
    dir: 'tool-cordis',
    ...
    note:
      'Not in any shipped tree (a deliberate opt-in — dynamic package code reaches the real runtime, see .agents/notes/implemented/feature/2026-07-08-self-referential-cordis-toolset.md). The toolset injects `ctx.dynamicCordisRunner` from `@cortex/cordis-host-runner`, which owns the definition registry and the vm sandbox; a composition missing it never activates the tools. A running package may register ADDITIONAL model-visible tools until it is stopped, undefined, or Cortex restarts; a full changed request header logs those tool-set changes.',
  },
  {
    pkg: '@cortex/tool-bash-persistent',
  ...
  {
    pkg: '@cortex/tool-str-replace-editor',
  ...
  },
  {
    ...
    note:
      'The read-before-write/edit policy is added by `@cortex/fs-observation-policy` (an `fs/*` event-gate plugin, no schema change); a deployment that loads these tools is expected to also load it. `read_image` is not registered without `ctx.attachments`; its schema is route-independent, and execution refuses unless the exact routed model declares image input.',
  },
  {
    pkg: '@cortex/tool-fs-search',
    dir: 'tool-fs-search',
  ...
  },
  {
    ...
    note:
      'The lsp tool keeps provider selection and language-server subprocesses behind ctx.lsp, so its model-visible schema stays stable across providers. Requires a registered provider (e.g. `@cortex/lsp-stdio`) at runtime; without one, a query returns the structured `LSP_UNAVAILABLE` error rather than changing the schema.',
  },
  {
    pkg: '@cortex/tool-ralph',
    dir: 'tool-ralph',
  ...
  },
  {
    ...
    async mount(ctx) {
      ...
      await ctx.plugin(SkillFileSystem, {
        cortexHome: resolve(root, '.tmp/tool-catalog/.cortex'),
  ...
  {
    pkg: '@cortex/tool-subagent-control',
  ...
  {
    pkg: '@cortex/tool-subagent-report',
