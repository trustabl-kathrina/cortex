// Real-host smoke: spawn `cortex web` with a real key, walk the full flow
// list in a real chromium, screenshot every screen into .artifacts/ for the
// figma comparison pass. Self-skips without CORTEX_API_KEY (repo e2e
...
function waitForReadyLine(child: ChildProcess): Promise<string> {
  return new Promise((resolveReady, reject) => {
    let out = ''
    const timer = setTimeout(() => { reject(new Error(`cortex web not ready in 90s; output:\n${out}`)) }, 90_000)
    const onData = (chunk: Buffer): void => {
      out += chunk.toString()
      const match = /cortex web: (http:\/\/[^\s]+)/.exec(out)
    ...
    child.once('exit', (code) => {
      clearTimeout(timer)
      reject(new Error(`cortex web exited early (code ${code}); output:\n${out}`))
    })
  })
}
...
// Readiness gate: `cortex web` serves every production manifest plugin; until every UI
...
describe('cortex web keyless CLI smoke', () => {
  it('listens on 127.0.0.1 by default', async () => {
    requireDist()
    const sessionsDir = mkdtempSync(join(tmpdir(), 'cortex-web-keyless-'))
    ...
    const child = spawn(
      ...
      ['--import', tsxLoader, join(REPO_ROOT, 'apps/cli/src/bin.ts'), 'web', '--port', '0'],
      {
        ...
        env: {
          ...process.env,
          CORTEX_API_KEY: 'keyless-web-no-call',
          CORTEX_HOME: join(sessionsDir, '.cortex'),
          CORTEX_AGENTS_HOME: join(sessionsDir, '.agents'),
  ...
  it('routes web runtime context and workspace instructions through the real CLI request', async () => {
    requireDist()
    const workspace = mkdtempSync(join(tmpdir(), 'cortex-web-workspace-'))
    ...
    const child = spawn(
      ...
      ['--import', tsxLoader, join(REPO_ROOT, 'apps/cli/src/bin.ts'), 'web', '--port', '0'],
      {
        ...
        env: {
          ...process.env,
          CORTEX_API_KEY: 'keyless-web-workspace',
          CORTEX_BASE_URL: `http://127.0.0.1:${address.port}`,
          CORTEX_HOME: join(workspace, '.cortex'),
          CORTEX_AGENTS_HOME: join(workspace, '.agents'),
          TSX_TSCONFIG_PATH: join(REPO_ROOT, 'tsconfig.json'),
        },
        stdio: ['ignore', 'pipe', 'pipe'],
      },
  ...
  it('retries a partial transport failure through the shipped Web composition', async () => {
    requireDist()
    const workspace = mkdtempSync(join(tmpdir(), 'cortex-web-retry-'))
    ...
    const child = spawn(
      ...
      ['--import', tsxLoader, join(REPO_ROOT, 'apps/cli/src/bin.ts'), 'web', '--port', '0'],
      {
        ...
        env: {
          ...process.env,
          CORTEX_API_KEY: 'keyless-web-retry',
          CORTEX_BASE_URL: `http://127.0.0.1:${address.port}`,
          CORTEX_HOME: join(workspace, '.cortex'),
          TSX_TSCONFIG_PATH: join(REPO_ROOT, 'tsconfig.json'),
        },
  ...
  it('CORTEX_TOOLS_MODE=code collapses the provider wire tools to run_code with the SDK prompt section', async () => {
    requireDist()
    const workspace = mkdtempSync(join(tmpdir(), 'cortex-web-code-mode-'))
    ...
    const child = spawn(
      ...
      ['--import', tsxLoader, join(REPO_ROOT, 'apps/cli/src/bin.ts'), 'web', '--port', '0'],
      {
        ...
        env: {
          ...process.env,
          CORTEX_API_KEY: 'keyless-web-code-mode',
          CORTEX_BASE_URL: `http://127.0.0.1:${address.port}`,
          CORTEX_TOOLS_MODE: 'code',
          CORTEX_HOME: join(workspace, '.cortex'),
          CORTEX_AGENTS_HOME: join(workspace, '.agents'),
          TSX_TSCONFIG_PATH: join(REPO_ROOT, 'tsconfig.json'),
        },
...
describe.skipIf(!process.env.CORTEX_API_KEY || notReady.length > 0)('web smoke (real host, real key)', () => {
  ...
  beforeAll(async () => {
    requireDist()
    sessionsDir = mkdtempSync(join(tmpdir(), 'cortex-web-w5-'))
    const port = await probeFreePort()
    // tsx boot mirrors the runtime half of the root cortex script. Isolate
  ...
  it('empty-state first send completes a real model round', async () => {
    ...
    // This scenario spawns its own server against a fresh $CORTEX_HOME with the
    // Cortex credential inherited from the environment, so no onboarding
    ...
    await page.waitForFunction(
      () => document.title !== 'Cortex' && document.title.endsWith(' — Cortex'),
      undefined,
      { timeout: 15_000 },
    )
    ...
    await page.waitForFunction(
      expected => document.title === `${expected} — Cortex`,
      durableTitle,
      { timeout: 15_000 },
    )
  ...
  it('bash differential rendering: tool row click leaves the default details column closed', async () => {
    ...
    const input = page.locator('textarea').first()
    await input.fill('Use the bash tool to run the command echo w5marker, then tell me the result')
    await input.press('Enter')
