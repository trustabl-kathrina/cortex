...
import { canonicalPath, writableRoots } from '@cortex/sandbox'
import { SessionId } from '@cortex/session'
// Empty type imports carry the tools/sandboxPolicy/approval Context merges.
import type {} from '@cortex/tools'
import type {} from '@cortex/sandbox-policy'
import type {} from '@cortex/user-approval'
import type {} from '@cortex/permission-presets'
import type {} from '@cortex/agent-presets'
...
/**
 * `glob` and `grep` come from `cortex-tool-fs-search`, which spawns the PACKAGED
...
it('assembles the shipped Web catalog, file-reference guidance, and confined access default', async () => {
  ...
  const commandHandle = await scaffold.ctx.agents.create({
    ...
    agentOptions: { provider: 'cortex-official', model: 'cortex-v4-flash' },
  })
  try {
