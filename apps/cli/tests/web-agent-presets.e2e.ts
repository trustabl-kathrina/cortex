...
import { boot, healProfilesModuleFallback, loadOverlayPatches } from '@cortex/app-boot'
import { provideCmdline } from '@cortex/cmdline'
import { SessionId } from '@cortex/session'
import type { Agent } from '@cortex/agent'
import type { PatchOptions } from '@cortex/cordis-plugin-include'
import { afterAll, beforeAll, describe, expect, it } from 'vitest'
import { settingsNamespace } from '@cortex/settings'
import { resolveSessionPreset, SETTINGS_NAMESPACE } from '@cortex/agent-presets'
import { applyChildComposition, childSessionMeta } from '@cortex/subagent'
import { CallId } from '@cortex/llm'
import type {} from '@cortex/compaction-basic'
import type {} from '@cortex/skill'
import type {} from '@cortex/tools'
// Type-only: resolves `ctx.get('sessionProjections')` and `ctx.get('tokenMeter')`.
import type {} from '@cortex/session-projection'
import type {} from '@cortex/token-meter'

const CONFIG_DIR = fileURLToPath(new URL('../config/', import.meta.url))
const REPO_ROOT = fileURLToPath(new URL('../../..', import.meta.url))
/** The shipped Web surface: the cortex-base and cortex-web-app bundle patches over an empty preset root. */
...
async function bootWeb(
  ...
  const patches: PatchOptions[] = [
    ...loadOverlayPatches('cortex-test', BASE_PATCH),
    ...loadOverlayPatches('cortex-test', WEB_PATCH),
    // The settings row defaults to `$CORTEX_HOME/settings.yaml`. Left alone it
    ...
    // storage-json's root is anchored to the real $CORTEX_HOME. Unpinned, this
    // file writes the developer's own `~/.cortex/storages/` — and then reads it
    ...
    { insert: [
      { id: 'directory-picker-browse', name: '@cortex/host-directory-picker-browse' },
      { id: 'ui-directory-picker-browse', name: '@cortex/client-ui-directory-picker-browse' },
    ] },
    // The roster AppCLIEntry would patch in; only the shipped root, so a
    // developer's own `~/.cortex/.preset` cannot change this test's outcome.
  ...
  return await boot('cortex-test', rootConfig, patches, (bootCtx) => {
...
let ctx: Context
beforeAll(async () => {
  const settingsFile = join(await mkdtemp(join(tmpdir(), 'cortex-web-presets-')), 'settings.yaml')
...
describe('the shipped Web composition', () => {
  ...
  it('merges the global skill layer into a preset agent\'s catalog, keeping local discovery preset-side', async () => {
    const proj = await mkdtemp(join(tmpdir(), 'cortex-preset-skill-proj-'))
    await mkdir(join(proj, '.cortex', 'skills', 'project-proof'), { recursive: true })
    await writeFile(join(proj, '.cortex', 'skills', 'project-proof', 'SKILL.md'), [
      '---',
      'name: project-proof',
    ...
    const handle = await ctx.agents.create({
      // Unique per run: the composition persists into the ambient Cortex home,
    ...
    try {
      ...
      expect((await ctx.skills.list({ cwd: proj })).map(skill => skill.name)).toEqual(['cortex-badge'])
      ...
      expect(scoped).toContain('cortex-badge')
      ...
      expect(JSON.stringify(loaded.content)).toContain('powered by cortex')
    } finally {
      await handle.dispose()
    }
  })
  ...
  it('shows a minimal agent the global layer but no loader tool', async () => {
    ...
    try {
      ...
      expect((await ctx.skills.list({ scope: handle.agent })).map(skill => skill.name)).toContain('cortex-badge')
...
describe('product subagent rows in user presets', () => {
  ...
  beforeAll(async () => {
    const root = await mkdtemp(join(tmpdir(), 'cortex-product-presets-'))
    ...
    productCtx = await bootWeb(settingsFile, [
      { insert: [
        { id: 'subagent-codex', name: '@cortex/subagent-codex' },
        { id: 'subagent-claude-code', name: '@cortex/subagent-claude-code' },
      ] },
      {
        id: 'agent-presets',
        config: {
...
describe('a launcher that configures no writable root', () => {
  ...
  // roster derives `<cortexHome>/.agent-presets` itself. `$CORTEX_HOME` is pointed
  ...
  beforeAll(async () => {
    const home = await mkdtemp(join(tmpdir(), 'cortex-preset-derived-'))
    previousHome = process.env.CORTEX_HOME
    ...
    const settingsFile = join(await mkdtemp(join(tmpdir(), 'cortex-preset-derived-settings-')), 'settings.yaml')
    await writeFile(settingsFile, '{}\n')
...
describe('authoring a preset on the shipped composition', () => {
  ...
  beforeAll(async () => {
    userRoot = join(await mkdtemp(join(tmpdir(), 'cortex-preset-authoring-')), 'profiles')
    const settingsFile = join(await mkdtemp(join(tmpdir(), 'cortex-preset-authoring-settings-')), 'settings.yaml')
    await writeFile(settingsFile, '{}\n')
  ...
  it('copies a shipped preset a session then really composes from', async () => {
    await authorCtx.agentPresets.copy('minimal', 'my-agent', 'My Mode')
