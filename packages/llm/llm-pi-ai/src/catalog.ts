 ...
 *
 * @module cortex-llm-pi-ai/catalog
 */
...
/**
 * The reasoning-dispatch wire formats a profile may name, most-reached first.
 * The rest of pi-ai's spellings are withheld: the chat-template ones drive
 * the request through `chatTemplateKwargs`, which this configuration does not
 * expose, and the vendor-bound ones belong to catalog routes whose entries
 * already carry them. `satisfies` pins every key to a spelling pi-ai really
 * accepts, so an upgrade that renames or drops one fails compilation here.
 */
...
} as const satisfies Partial<Record<PiThinkingFormat, true>>
...
/**
 * The installed catalog providers the settings page proposes on its own — the
 * routes a fresh key can be entered for. Catalog *membership* is unaffected:
...
const OFFERED_CATALOG_PROVIDERS = {
  ...
  'xai': true,
} as const satisfies Partial<Record<BuiltinProvider, true>>

/**
 * Whether the settings page proposes one installed catalog provider on its
 * own; see {@link OFFERED_CATALOG_PROVIDERS}.
 * @param provider - provider route key.
 * @returns whether the route is proposed; false for a route pi-ai does not ship.
 */
export function catalogProviderOffered(provider: string): boolean {
  return Object.hasOwn(OFFERED_CATALOG_PROVIDERS, provider)
}
 ...
 * `providers: { openai: { apiKeyEnv: … } }` profile working untouched.
