 ...
 * ```
 *
 * @module @cortex/llm-pi-ai
 */

import type { Context } from '@cortex/cordis'
import { launchEnvironmentOf } from '@cortex/launch-environment'
import { assertUsableApiKey, LlmError } from '@cortex/llm'
import type { AdapterRegistrationHandle, DirectoryRegistrationHandle, LlmConfigurableProvider } from '@cortex/llm'
import { deepEqualJson, installSettingsSection, settingsNamespace } from '@cortex/settings'
import { PiAiAdapter } from './adapter.ts'
import { catalogProviderIds, catalogProviderOffered, catalogProviderTakesApiKey } from './catalog.ts'
...
function directoryEntries(
  ...
  // answers what pi-ai ships. The same holds for a shipped route this build
  // does not propose on its own (`catalogProviderOffered`): a profile naming
  // it still resolves against the catalog and stays on the page.
  for (const provider of catalog) {
    if (catalogProviderOffered(provider) && catalogProviderTakesApiKey(provider)) declare(provider, provider)
  }
...
export function apply(ctx: Context, config: Config): void {
  ...
  const ensureDirectory = (): void => {
    ...
    // family already declares (a profile keyed `cortex-official`) would
