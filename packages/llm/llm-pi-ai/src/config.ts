 ...
 *
 * @module cortex-llm-pi-ai/config
 */
...
import { resolveRetryPolicy, RetryPolicySchema } from '@cortex/llm'
import type { ResolvedRetryPolicy, RetryPolicyConfig } from '@cortex/llm'
...
function rejectRemovedFields(provider: string, source: PiAiProviderProfile): void {
  ...
  if ('maxRetries' in legacy || 'maxRetryDelayMs' in legacy) {
    throw new Error(
      ...
      + ' compose agent recovery with cortex-llm-retry',
    )
  }
}
