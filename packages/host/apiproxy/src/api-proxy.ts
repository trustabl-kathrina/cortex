...
import { installModelSelection } from '@cortex/agent'
import type { Agent, ModelSelection, ModelSelectionRef, AgentOptions, AgentStatus } from '@cortex/agent'
import type {} from '@cortex/agent-presets/types'
import { AttachmentError } from '@cortex/attachment'
import type { ImageAttachmentRef } from '@cortex/attachment'
import { contentHasImage, createUserMessage, freezeMessage, ReasoningEffortId } from '@cortex/llm'
import { errorChain } from '@cortex/llm'
import type { ContentBlock, MessageSource } from '@cortex/llm'
import { isAppendSurfaceEvent, isJsonValue } from '@cortex/session'
import type { JsonValue, Session, SessionEvent, SessionEventMap, SessionHeader, SessionId, UserMessage } from '@cortex/session'
import type { SessionPersistence } from '@cortex/session-persistence'
import { SessionQueryError, type SessionSearchCursor } from '@cortex/session-query'
import { SubagentError } from '@cortex/subagent'
import type { SubagentListEntry as CatalogSubagentListEntry } from '@cortex/subagent'
import { isUserInvocable } from '@cortex/skill'
import type { Workspace, WorkspaceRecord } from '@cortex/workspace'
import {
  workspaceDomainState, workspaceRecord, WorkspaceId as brandWorkspaceId,
  WorkspaceMoveInvalidError, WorkspaceOrderInvalidError, WorkspaceUnknownSessionError,
} from '@cortex/workspace'
...
} from '@cortex/agent-presets'
import type { PresetBearingSession } from '@cortex/agent-presets'
...
import type { SessionRawArtifact } from '@cortex/session-persistence'
import {
...
import type {} from '@cortex/jobs'
import type { JobSnapshot } from '@cortex/jobs'
// Type-only: resolves `ctx.get('sessionProjectionCache')` (the cold listing column).
import type {} from '@cortex/session-projection-cache'
// GoalError narrows domain rejections to their stable codes at the wire boundary.
import { GoalError } from '@cortex/goal'
import type { GoalRef as CoreGoalRef } from '@cortex/goal'
...
import type {} from '@cortex/cordis-host-runner/types'
import type {} from '@cortex/skill'
// The settings/credentials seams: brand guards run at this wire boundary; the
// service reads stay optional (`ctx.get`) so a composition without either
// provider still serves every other domain.
import { SettingsConflictError, settingsNamespace } from '@cortex/settings'
import type { SettingsDescriptor, SettingsNamespace, SettingsPathOp } from '@cortex/settings'
import { credentialRef } from '@cortex/credentials'
// Value edge: the rename impl narrows the title service's validation failure; the import also resolves `ctx.get('sessionTitle')`.
import { SessionTitleInvalidError } from '@cortex/session-title'
import type { CallId } from '@cortex/llm/brand'
import type { ScopeKey } from '@cortex/scope'
import type { ApprovalOutcome, ApprovalRequestId } from '@cortex/user-approval'
// Side-effect type import: resolves the `approval/request` waterfall and
// `ctx.get('approval')` without a value dependency on the seam (optional composition).
import type {} from '@cortex/user-approval'
...
} from '@cortex/user-questions'
import { UserQuestionError } from '@cortex/user-questions'
import { DirectoryPickerError } from '@cortex/host-directory-picker'
import {
 ...
 * plugin-owned entries (`agent-loop`, `shell`) are the
...
const WEB_SETTINGS_NAMESPACES = [
  'agent-loop', 'shell', 'locale', 'permission', 'ui-conversation', 'ui-theme', 'atlassian',
] as const
...
function projectionsUnavailableError(): RpcError {
  return {
    code: 'internal',
    message: 'subagent catalog is unavailable: this deployment does not mount the sessionProjections registry (load @cortex/session-projection)',
    details: {},
  }
}
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  // attachment Service Definition: cortex-llm depends on cortex-attachment, so the
  ...
  function goalServiceFor(agent: Agent): NonNullable<ReturnType<typeof ctx.get<'goals'>>> | { error: RpcError } {
    ...
    if (goals === undefined) {
      return { error: { code: 'internal', message: 'goal service is absent: neither this session\'s agent preset nor the host composition mounts @cortex/goal', details: {} } }
    }
    return goals
  }
  ...
  function settingsAbsent(): RpcError {
    return { code: 'internal', message: 'settings service is absent: this deployment does not mount a settings provider (e.g. @cortex/settings-file) in its composition', details: {} }
  }
  ...
  function credentialsAbsent(): RpcError {
    return { code: 'internal', message: 'credentials service is absent: this deployment does not mount a credential provider (e.g. @cortex/credentials-local) in its composition', details: {} }
  }
  ...
  return {
    sessions: {
      ...
      async search(request, signal) {
        ...
        if (sessionQuery === undefined) {
          return err(request, {
            code: 'internal',
            message: 'session search is unavailable: this deployment does not mount @cortex/session-query',
            details: {},
          })
        }
        try {
    ...
    agentPresets: {
      ...
      // Authoring is privileged (see PRIVILEGED_METHODS in cortex-client-connection):
    ...
    skills: {
      ...
      async list(request) {
        ...
        // composition mounts cortex-skill, not an empty catalog. `ctx.get` also
        ...
        if (skillRegistry === undefined) {
          return err(request, { code: 'internal', message: 'skill registry is absent: neither this session\'s agent preset nor the host composition mounts @cortex/skill', details: {} })
        }
