...
export const en = {
  ...
  lockedNotice: 'Provider setup is restricted on this deployment: only a local LiteLLM gateway (e.g. http://127.0.0.1:4000/v1) can be configured here. Other provider routes are managed by the administrator in settings.yaml.',
  lockedAdd: 'Adding catalog providers is disabled on this deployment. Declare a local LiteLLM gateway with "Add a custom provider" instead.',
  lockedRow: 'This provider is managed by the administrator and cannot be changed here.',
  lockedCustomHint: 'Declare your local LiteLLM gateway (localhost only).',
  lockedEndpoint: 'Only a localhost LiteLLM endpoint can be configured here, e.g. http://127.0.0.1:4000/v1.',
  ...
  onboardingDescription: 'Configure the official Cortex provider to start building.',
